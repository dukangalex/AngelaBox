...
1. 把要串联的配置设为当前配置。
2. 打开 **工具 → 链式代理**。页面说明都在右上角 ⓘ：非中国流量必须经落地出口；脚本只对前置生效，落地配置勿开脚本。
...
链式模式下脚本仍然生效，但**只改写前置（当前）配置**；落地配置上开启的脚本不会执行，请不要把脚本开在落地配置上。默认脚本覆盖原分组与分流，并注入当前镜像上存在的规则集（含流媒体、AI、社交、游戏等；镜像上 404 的条目不会写入）。若本机仍保存着旧版默认脚本，启动和打开脚本列表时会自动换成当前版本。启动时也会丢掉会 404 的远程规则集，避免整次启动失败。脚本编辑为全屏页。
