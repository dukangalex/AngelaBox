...
1. 把要串联的配置设为当前配置。
2. 打开 **工具 → 链式代理**。页面说明都在右上角 ⓘ：非中国流量必须经落地出口；链式开启后当前配置的脚本会自动关闭。
...
路径：**工具 → 脚本**。这里是脚本库，可导入多条 `function main(config)` 脚本（代码 / URL / 文件），以及自带的「默认脚本」。导入后**不会**自动套到所有配置；必须在**编辑配置**页（或仪表配置列表的「脚本」菜单）为需要的配置打开。未单独打开的配置不跑脚本。打开后脚本**覆盖**该配置原有分组和分流，只保留节点，不写回订阅。中国直连、防 WebRTC、禁用 QUIC 等开关在脚本之后仍然生效；广告拦截开关若发现脚本已经分流广告，就不会再强制拒绝。URL 导入的脚本可在列表菜单里「同步」上游。列表里的开关是该脚本的总开关。

链式开启后**当前配置的脚本会自动关闭**，两者不同时生效；取消链式后如需脚本，到编辑配置页再打开。默认脚本覆盖原分组与分流：节点选择 / 自动选择 / 广告拦截 / 远控工具在前，地区组在最后。广告拦截与远控可在 REJECT-DROP、REJECT、DIRECT（远控还有国外服务）之间切换；内核没有独立拦截出站，这两项在本机黑洞上实现。规则集只用镜像上存在的文件。若本机仍保存着旧版默认脚本，启动和打开脚本列表时会自动换成当前版本。启动时也会丢掉会 404 的远程规则集，并剥掉直连出站上已删除的覆写字段，避免无法启动。脚本编辑为全屏页。

配置覆盖里的广告拦截若发现脚本已经把广告分到分组，就不会再强制拒绝，避免两套策略打架。中国直连、防 WebRTC、禁用 QUIC 等开关仍会叠加在脚本之后。
