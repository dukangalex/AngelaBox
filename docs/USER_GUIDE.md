...
链式开启后**前置订阅的脚本仍然生效**，顺序是：脚本改分组和分流 → 链式把入口接到落地。落地配置上的脚本不会套到当前配置。默认脚本对齐原版机场覆写：**国内 IP/域名先直连**，国外域名走代理。非国内的 A/AAAA 用 fake-ip，不必先等节点上的 DNS；ECH 查询改走直连。系统 DNS 会先劫持；国外 QUIC/HTTP3 拦截后回落 TCP，YouTube / Gemini / AI Studio 才能打开。🔰 节点选择 / ♻️ 自动选择 / 广告拦截 / 远控工具在前，地区组在最后。广告拦截与远控可在 REJECT-DROP、REJECT、DIRECT（远控还有国外服务）之间切换；节点选择不提供 DIRECT。叶节点会去掉 detour / dialer-proxy，避免订阅把链式带进来。内核没有独立拦截出站，广告与远控在本机黑洞上实现。规则集只用镜像上存在的文件。若本机仍保存着旧版默认脚本，启动和打开脚本列表时会自动换成当前版本。启动时会把会 404 的远程规则集换成官方地址（保留原来的名字），并剥掉直连出站上已删除的覆写字段，避免无法启动。脚本编辑为全屏页。
