...
路径：**工具 → 脚本**。这里是脚本库，可导入多条 `function main(config)` 脚本（代码 / URL / 文件），以及自带的「默认脚本」。每个配置在**编辑配置**页（或仪表配置列表的「脚本」菜单）单独选择是否启用、启用哪几条。未单独设置的配置继承列表里的默认开关。脚本只改**运行时** sing-box JSON，不写回订阅。内置开关（中国直连、广告拦截、防 WebRTC 等）在脚本之后仍然生效。URL 导入的脚本可在列表菜单里「同步」上游。

链式模式下脚本仍然生效，但**只改写前置（当前）配置**；落地配置上开启的脚本不会执行，请不要把脚本开在落地配置上。默认脚本只注入当前镜像上存在的规则集。若本机仍保存着旧版默认脚本（含失效规则集），启动和打开脚本列表时会自动换成当前版本。启动时也会丢掉会 404 的远程规则集，避免整次启动失败。脚本编辑为全屏页。
