...
内置默认脚本是 `app/src/main/assets/scripts/airport-region.js`（`overlay-revision: 22`）。它按全地区识别分组。地区自动选择是地区选择组里的一个成员，分组列表不单独再列一张卡片。没有负载均衡 / 故障转移。延迟挑选用 urltest，手选用 selector。设置里的配置覆盖开关作用在这份脚本上。不要再把同一份逻辑当成第二个脚本绑到同一配置。
