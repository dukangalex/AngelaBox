...
• **Windows ZIP 发布：** 图形安装器发布为 `AngelaBox-v1.0.65-windows-amd64.zip`，其中含对应的安装 `.exe`，并生成 SHA-256 校验文件，避免以裸 `.exe` 触发浏览器的 HTTP 下载阻断。命令行内核继续使用独立且不冲突的 `AngelaBox-windows-amd64.zip`（另有 arm64 / 386），其中含 `sing-box.exe`、许可证和使用说明。
