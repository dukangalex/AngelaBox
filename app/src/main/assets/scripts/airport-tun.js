 ...
 * airport-tun-revision: 4
 * 从机场覆写改写到 sing-box：地区分组、自动选择、负载均衡、故障转移、
 * 服务分组、强制 TUN、本机 mixed。不写链式，叶节点去掉 detour / dialer-proxy。
 * 国外 A/AAAA 用 fake-ip，不必先等节点上的 DNS。DNS 用 TCP/UDP，不用 DoH。
 * 不拒绝 HTTPS/SVCB，也不丢弃 UDP 443。
 * Clash 的 proxies / proxy-groups / RULE-SET 不能在本引擎跑；本脚本读写
 * 官方 sing-box JSON（outbounds / route / inbounds / dns）。
 * 配置覆盖开关通过全局 overlay 控制，默认全开。不要在脚本里改开关。
 * 测速与规则集只用 HTTPS。叶节点去掉 detour / dialer-proxy。
 * 不要和默认脚本绑在同一配置上。function main(config)。
 */
function main(config) {
  ...
  addService("🤖 Claude AI", serviceMembers([pickSelect, AUTO_NAME], true));
  ...
  for (var ib = 0; ib < inbounds.length; ib++) {
    ...
    if (ity === "tun") {
      ...
      } else if (!inbound.inet6_address && !inbound.address) {
    ...
    if (ity === "mixed") {
      ...
      if (!inbound.listen_port) inbound.listen_port = 17890;
    }
  }
  if (!hasTun) {
    var tunIn = {
      type: "tun",
      tag: "tun-in",
      address: disableIpv6 ? ["172.19.0.1/30"] : ["172.19.0.1/30", "fdfe:dcba:9876::1/126"],
      auto_route: true,
      sniff: true,
      mtu: 1500
    };
